 ...
 */
public class TurtleWriter<S extends Resource<?, ?, ?>, P extends Property, O extends Literal<?>> {
    ...
    private void writeNamespaces(URINamespaceContext context, Resource<S, P, O> resource) throws IOException {
        ...
        while (it.hasNext()) {
            ...
            s = context.inContext(stmt.getPredicate().getURI());
            if (s != null) {
                newContext.addNamespace(s[0], s[1]);
            }
            O o = stmt.getObject();
    ...
    private void openResource(BlankNode<S, P, O> node) throws IOException {
        writer.write("[");
        writer.write(LF);
        writeIndent(1);
        subjects.push(statement.getSubject());
        predicates.push(statement.getPredicate());
    }
    ...
    private void closeSubject() throws IOException {
        if (sameSubject) {
            closeResource();
            if (statement.getSubject().equals(lastSubject)) {
                writer.write(";");
                writer.write(LF);
                writeIndent(1); // ?
            } else {
