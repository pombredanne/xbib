 ...
 */
public class Query implements ResultOperator<SQLSession, ResultSet> {
    ...
    private ResultProcessor<ResultSet> processor;
    ...
    public void execute(SQLSession session) throws IOException {
        ...
        try {
            ...
            try {
                if (processor != null && !(processor instanceof SQLResultWithDelayedCloseProcessor)) {
                    if (results != null) {
                        results.close();
                    }
                }
     ...
     *
     * @throws SQLException if string can't be set
     */
    ...
    private void setString(PreparedStatement p, int pos, String s) throws SQLException {
        if (s == null) {
            p.setNull(pos, java.sql.Types.VARCHAR);
            return;
        }
