...
public class AsyncClientTest {
    ...
    public void testMultiClient() throws Exception {
        for (String s : Arrays.asList("Bielefeld", "Gent", "Lund")) {
            SRUClient client = SRUClientFactory.getClient(s);
            String query = "title = linux";
            int from = 0;
            int size = 10;
            SearchRetrieve request = new SearchRetrieve().setURI(client.getURI())
                    .setUsername(client.getUsername())
                    .setPassword(client.getPassword()).setVersion(client.getVersion()).setRecordPacking(client.getRecordPacking()).setRecordSchema(client.getRecordSchema()).setQuery(query).setStartRecord(from).setMaximumRecords(size);
