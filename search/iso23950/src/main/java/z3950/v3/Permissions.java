...
public final class Permissions extends ASN1Any {
    ...
    /**
     * Default constructor for a Permissions.
     */
    ...
    public Permissions(BEREncoding ber, boolean check_tag)
    ...
    ber_decode(BEREncoding ber, boolean check_tag)
        ...
        try {
            ...
            throw new ASN1EncodingException
                    ("Zebulun Permissions: bad BER form\n");
        }

        // Prepare to decode the components

        int num_parts = ber_cons.number_components();
        value = new Permissions1[num_parts];
        int p;
        for (p = 0; p < num_parts; p++) {
            value[p] = new Permissions1(ber_cons.elementAt(p), true);
        }
    }
