...
public final class OtherInformation extends ASN1Any {
    ...
    public OtherInformation(BEREncoding ber, boolean check_tag)
    ...
    ber_decode(BEREncoding ber, boolean check_tag)
        ...
        if (check_tag) {
            if (ber.tag_get() != 201 ||
                ...
                throw new ASN1EncodingException
                        ("Zebulun: OtherInformation: bad BER: tag=" + ber.tag_get() + " expected 201\n");
            }
        }
        ...
        try {
            ...
            throw new ASN1EncodingException
                    ("Zebulun OtherInformation: bad BER form\n");
        }

        // Prepare to decode the components

        int num_parts = ber_cons.number_components();
        value = new OtherInformation1[num_parts];
        int p;
        for (p = 0; p < num_parts; p++) {
            value[p] = new OtherInformation1(ber_cons.elementAt(p), true);
        }
    }
    ...
    ber_encode()
            throws ASN1Exception {
        return ber_encode(BEREncoding.CONTEXT_SPECIFIC_TAG, 201);
    }
