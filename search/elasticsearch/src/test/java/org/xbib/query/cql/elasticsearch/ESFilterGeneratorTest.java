...
public class ESFilterGeneratorTest extends Assert {
    ...
    public void testSimpleTermFilter() throws Exception {
        String cql = "Jörg";
        CQLParser parser = new CQLParser(new StringReader(cql));
        parser.parse();
        ESFilterGenerator generator = new ESFilterGenerator();
        parser.getCQLQuery().accept(generator);
        String json = generator.getResult().string();
        logger.info("{} --> {}", cql, json);
        assertEquals(json, "{\"query\":{\"term\":{\"_all\":\"Jörg\"}}}");
    }
    ...
    public void testFieldTermFilter() throws Exception {
        ...
        String json = generator.getResult().string();
        logger.info("{} --> {}", cql, json);
        assertEquals(json, "{\"query\":{\"term\":{\"dc:type.xbib:type\":\"electronic\"}}}");
    }

    @Test
    public void testMulitFieldTermFilter() throws Exception {
        ...
        String json = generator.getResult().string();
        logger.info("{} --> {}", cql, json);
        assertEquals(json, "{\"query\":{\"bool\":{\"must\":{\"term\":{\"dc:type.xbib:type\":\"electronic\"}},\"must\":{\"term\":{\"dc:date.xbib:date\":\"2013\"}}}}}");
    }
