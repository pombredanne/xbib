...
public class Elasticsearch implements IElasticsearch {
    ...
    private final static ESLogger logger = ESLoggerFactory.getLogger(Elasticsearch.class.getName());
    ...
    @Override
    public synchronized Elasticsearch newClient(URI uri) {
        if (client != null) {
            client.close();
            client = null;
        }
    ...
    /**
     * Create settings for searching an Elasticsearch
     *
     * @param uri
     * @return the settings
     */
    ...
    protected static URI findURI() {
        ...
        try {
            ...
            InputStream in = Elasticsearch.class.getResource("/org/xbib/elasticsearch/cluster.properties").openStream();
            p.load(in);
            in.close();
