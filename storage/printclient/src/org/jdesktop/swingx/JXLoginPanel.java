/*
 * $Id: JXLoginPanel.java,v 1.3 2005/06/27 08:26:58 kleopatra Exp $
 *
...
/**
 * <p>JXLoginPanel is a JPanel that implements a Login dialog with support for
 * saving passwords supplied for future use in a secure manner. It is intended
 * to work with <strong>LoginService</strong> and <strong>PasswordStore</strong>
 * to implement the authentication.</p>
 *
 * <p> In order to perform the authentication, <strong>JXLoginPanel</strong>
 * calls the
 * <code>authenticate</code> method of the <strong>LoginService </strong>. In
 * order to perform the persistence of the password,
 * <strong>JXLoginPanel</strong> calls the put method of the
 * <strong>PasswordStore</strong> object that is supplied. If the
 * <strong>PasswordStore</strong> is
 * <code>null</code>, then the password is not saved. Similarly, if a
 * <strong>PasswordStore</strong> is supplied and the password is null, then the
 * <strong>PasswordStore</strong> will be queried for the password using the
 * <code>get</code> method.
 *
 * Changes by Shai: Clarified the save mode a little bit including hiding the
 * save checkbox when there is no password store. Changed the class to derive
 * from JXImagePanel to make customization easier (need to check my ImagePanel
 * which has some technical advantages). Removed the static keyword from the
 * ok/cancel buttons since this can cause an issue with more than one login
 * dialogs (yes its an unlikely situation but documenting this sort of behavior
 * or dealing with one bug resulting from this can be a real pain!). Allowed the
 * name field to be represented as a text field when there is no password store.
 * Rewrote the layout code to mostly work with a single container. Removed
 * additional dialogs for progress and error messages and incorporated their
 * functionality into the main dialog. Allowed for an IOException with a message
 * to be thrown by the login code. This message is displayed to the user when
 * the login is stopped. Removed repetetive code and moved it to a static block.
...
public class JXLoginPanel extends JXImagePanel {
    ...
    /**
     * The JXLoginPanel can attempt to save certain user information such as the
     * username, password, or both to their respective stores. This type
     * specifies what type of save should be performed.
     */
    public static enum SaveMode {

        NONE, USER_NAME, PASSWORD, BOTH
    };
    /**
    ...
    /**
     * This is the area in which the name field is placed. That way it can
     * toggle on the fly between text field and a combo box depending on the
     * situation.
     */
    private JXPanel namePanel;
    /**
     * If a UserNameStore is used, then this combo box is presented allowing the
     * user to select a previous login name, or type in a new login name
     */
    private JComboBox nameCombo;
    /**
     * If a UserNameStore is not used, then this text field is presented
     * allowing the user to simply enter their user name
     */
    ...
    /**
     * Check box presented if a PasswordStore is used, allowing the user to
     * decide whether to save their password
     */
    private JCheckBox saveCB;
    /**
     * The LoginService to use. This must be specified for the login dialog to
     * operate. If no LoginService is defined, a default login service is used
     * that simply allows all users access. This is useful for demos or
     * prototypes where a proper login server is not available.
     */
    private LoginService loginService;
    /**
     * Optional: a PasswordStore to use for storing and retrieving passwords for
     * a specific user.
     */
    private PasswordStore passwordStore;
    /**
     * Optional: a UserNameStore to use for storing user names and retrieving
     * them
     */
    ...
    /**
     * Tracks the cursor at the time that authentication was started, and
     * restores to that cursor after authentication ends, or is cancelled;
     */
    ...
    static {
        ...
        String lookup;
        ResourceBundle res = ResourceBundle.getBundle("org.jdesktop.swingx.auth.resources");
    ...
    /**
     * The save mode indicates whether the "save" password is checked by
     * default. This method makes no difference if the passwordStore is null.
     *
     * @param saveMode The saveMode to set either SAVE_NONE, SAVE_PASSWORD or
     * SAVE_USERNAME
     */
...
//            finishedLogin(false);
    ...
    /**
     * Simple login service that allows everybody to login. This is useful in
     * demos and allows us to avoid having to check for LoginService being null
     */
