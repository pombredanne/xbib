...
public class ResourceGenerator implements LIA, Visitor {
    ...
    public void visit(Access access) {
        ...
        resource.add(NAME, access.getName());
        resource.add(AUTHORITY, access.getAuthority().getToken());
    ...
    public void visit(Library library) {
        ...
        resource.add(NAME, library.getName());
        resource.add(AUTHORITY, library.getAuthority().getToken());
        resource = stack.pop();
    }

    @Override
    public void visit(Service service) {
        resource.add(SERVICE, service.getToken());
    }
    ...
    public void visit(Item item) {
        ...
        resource.add(ITEM_ID, item.getIdentifier());
        resource.add(ITEM_TYPE, item.getType().getToken()  );
        resource.add(ITEM_PREFERRED_TRANSPORT, item.getTransport().getToken() );
        resource.add(ITEM_PREFERRED_DELIVERY, item.getDelivery().getToken() );
        if (item instanceof PhysicalItem) {
            PhysicalItem physical = (PhysicalItem) item;
            resource.add(ITEM_COLLECTION, physical.getCollection());
            resource.add(ITEM_DESCRIPTION, physical.getDescription());
            resource.add(ITEM_SHELFMARK, physical.getShelfMark());
            for (String number : physical.getNumbers()) {
                resource.add(ITEM_NUMBER, number);
            }
        } else if (item instanceof LocatorItem) {
            LocatorItem locatorItem = (LocatorItem) item;
            Locator locator = locatorItem.getLocator();
            resource.add(ITEM_ADDRESS, locator.getAddress().toExternalForm()  );
            resource.add(ITEM_LABEL, locator.getLabel());
        }
        resource = stack.pop();
    }
