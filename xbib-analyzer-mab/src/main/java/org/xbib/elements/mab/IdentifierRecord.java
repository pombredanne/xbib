...
public class IdentifierRecord extends MABElement {
    ...
    public void build(MABBuilder b, FieldCollection key, String value) {
        value = value.trim();
        b.context().resource().id(URI.create("http://xbib.org#" + value)); // temporal ID
        b.context().getResource(b.context().resource(), IDENTIFIER).property(XBIB_IDENTIFIER_AUTHORITY_MAB, value);
    }
