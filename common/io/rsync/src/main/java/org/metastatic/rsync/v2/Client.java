...
public class Client
     ...
     * @param optind The index in the arguments array where non-option arguments begin.
    ...
    public int startShellClient(String path, String[] argv, int optind) {
        ...
        Process p = null;
        {
            ...
            try {
                ...
                logger.error("could not execute " + options.shell_cmd);
                logger.error(ioe.getMessage());
                return 1;
            }
        }
        ...
        try {
            ...
            logger.error("error exchanging protocol version");
            logger.error(ioe.getMessage());
            return 1;
        }
        if (remoteVersion < MIN_PROTOCOL_VERSION
                || remoteVersion > MAX_PROTOCOL_VERSION) {
            logger.error("protocol version mismatch (is your shell clean?)");
            p.destroy();
            return 1;
        }
    ...
    public static void main(String[] argv) {
        ...
        try {
            config.strongSum = MessageDigest.getInstance("MD5");
        ...
        if (options.am_server) {
            // FIXME.
            logger.error(PROGNAME + ": not implemented.");
            ret = 1;
        } else {
