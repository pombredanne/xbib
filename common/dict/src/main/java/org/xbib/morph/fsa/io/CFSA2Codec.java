...
import org.xbib.morph.fsa.io.FSAUtils.IntIntHolder;
...
import static org.xbib.morph.fsa.CFSA2.BIT_FINAL_ARC;
import static org.xbib.morph.fsa.CFSA2.BIT_LAST_ARC;
import static org.xbib.morph.fsa.CFSA2.BIT_TARGET_NEXT;
import static org.xbib.morph.fsa.FSAFlags.FLEXIBLE;
import static org.xbib.morph.fsa.FSAFlags.NEXTBIT;
import static org.xbib.morph.fsa.FSAFlags.NUMBERS;
import static org.xbib.morph.fsa.FSAFlags.STOPBIT;

/**
 * Reads CFSA2 graphs from input streams to memory and
 * writes {@link org.xbib.morph.fsa.FSA} graphs to {@link org.xbib.morph.fsa.CFSA2}.
 * <p/>
 * <p>
 * It is possible to write the automaton with numbers required for perfect
 * hashing. See {@link #withNumbers()} method.
 * </p>
 *
 * @see org.xbib.morph.fsa.CFSA2
 */
public final class CFSA2Codec implements FSACodec {
    /**
     * Supported flags.
     */
    ...
    /**
     * <code>true</code> if we should write with numbers.
     *
     * @see #withNumbers()
     */
     ...
     */
    @Override
    public <T extends InputStream> CFSA2 read(T in) throws IOException {
        CFSA2 cfsa2 = new CFSA2();
        ...
        if (version != CFSA2.VERSION) {
            throw new IOException("This class can only read FSA version: " + CFSA2.VERSION);
        }

        // Read flags.
        short flagBits = FileUtils.readShort(in);
        EnumSet<FSAFlags> flags = EnumSet.noneOf(FSAFlags.class);
        ...
        cfsa2.hasNumbers(flags.contains(FSAFlags.NUMBERS));

        int labelMappingSize = FileUtils.readByte(in) & 0xff;
        byte[] labelMapping = new byte[labelMappingSize];
        FileUtils.readFully(in, labelMapping);
        cfsa2.setLabelMapping(labelMapping);

        cfsa2.setArcs(FileUtils.readFully(in));
        return cfsa2;
    }

    /**
     * Serializes any {@link org.xbib.morph.fsa.FSA} to {@link org.xbib.morph.fsa.CFSA2} stream.
     *
     * @return Returns <code>os</code> for chaining.
     * @see #withNumbers
     */
    @Override
    public <T extends OutputStream> T write(final FSA fsa, T os) throws IOException {
        /*
        ...
        writeShort(os, FSAFlags.asShort(fsaFlags));
    ...
    private IntArrayList linearize(final FSA fsa) throws IOException {
        ...
        //sublist = sublist.subList(0, cutAt);
        linearizeAndCalculateOffsets(fsa, sublist, linearized, offsets);
    ...
    private IntIntOpenHashMap computeInlinkCount(final FSA fsa) {
        ...
        while (!nodes.isEmpty()) {
            ...
            for (int arc = fsa.getFirstArc(node); arc != 0; arc = fsa.getNextArc(arc)) {
                if (!fsa.isArcTerminal(arc)) {
                    final int target = fsa.getEndNode(arc);
                    inlinkCount.put(target, inlinkCount.containsKey(target) ? 1 + inlinkCount.get(target) : 1);
                    if (!visited.get(target)) {
                        nodes.push(target);
                    }
                }
    ...
    private int emitNodes(FSA fsa, OutputStream os, IntArrayList linearized) throws IOException {
        ...
        for (int i = 0; i < linearized.size(); i++) {
            final int state = linearized.get(i);
            final int nextState = i + 1 < max ? linearized.get(i + 1) : NO_STATE;
    ...
    /** */
    @Override
    public CFSA2Codec withFiller(byte filler) {
        throw new UnsupportedOperationException("CFSA2 does not support filler. Use .info file.");
    }

    /** */
    @Override
    public CFSA2Codec withAnnotationSeparator(byte annotationSeparator) {
     ...
     *
     */
    static void writeShort(OutputStream os, short v) throws IOException {
        os.write((v >>> 8) & 0xff);
        os.write(v & 0xff);
    }
