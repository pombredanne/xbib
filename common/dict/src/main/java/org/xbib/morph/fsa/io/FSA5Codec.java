...
import static org.xbib.morph.fsa.FSAFlags.SEPARATORS;
import static org.xbib.morph.fsa.FSAFlags.STOPBIT;

/**
 * Serializes in-memory {@link org.xbib.morph.fsa.FSA} graphs to a binary format compatible with
 ...
 * </p>
 *
 * @see org.xbib.morph.fsa.FSA5
 */
public final class FSA5Codec implements FSACodec {
    ...
    /**
     * @see org.xbib.morph.fsa.FSA5#filler
     */
     ...
     */
    public <T extends InputStream> FSA5 read(T fsaStream) throws IOException {
        // Read the header first.
        final FSAHeader header = FSAUtils.readHeader(fsaStream);
		 ...
		 */
        EnumSet<FSAFlags> flags = EnumSet.of(FLEXIBLE, STOPBIT, NEXTBIT);
        if ((header.gtl & 0xf0) != 0) {
            flags.add(NUMBERS);
        }
        fsa5.setFlags(flags);
        fsa5.setNodeDataLength((header.gtl >>> 4) & 0x0f);
        fsa5.setGotoLength(header.gtl & 0x0f);
        fsa5.setFiller(header.filler);
        fsa5.setAnnotation(header.annotation);
        fsa5.setArcs(FileUtils.readFully(fsaStream));
        return fsa5;
    }
     ...
     */
    @Override
    public FSA5Codec withFiller(byte filler) {
     ...
     */
    @Override
    public FSA5Codec withAnnotationSeparator(byte annotationSeparator) {
     ...
     */
    @Override
    public <T extends OutputStream> T write(final FSA fsa, T os)
            throws IOException {
