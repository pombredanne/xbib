...
import org.xbib.morph.fsa.FSA5Test;
...
public abstract class WriterTestBase extends Assert {
    ...
    public void test_abc() throws IOException {
        testBuiltIn(createCodec().read(FSA5Test.class.getResourceAsStream("abc.fsa")));
    }
    ...
    public void test_minimal() throws IOException {
        testBuiltIn(createCodec().read(FSA5Test.class.getResourceAsStream("minimal.fsa")));
    }
    ...
    public void test_minimal2() throws IOException {
        testBuiltIn(createCodec().read(FSA5Test.class.getResourceAsStream("minimal2.fsa")));
    }
    ...
    public void test_en_tst() throws IOException {
        testBuiltIn(createCodec().read(FSA5Test.class.getResourceAsStream("en_tst.dict")));
    }
    ...
    private void checkSerialization(byte[][] input, FSA root)
            throws IOException {
        checkSerialization0(createCodec(), input, root);
        if (createCodec().getFlags().contains(FSAFlags.NUMBERS)) {
            checkSerialization0(createCodec().withNumbers(), input, root);
        }
    }

    /** */
    private void checkSerialization0(FSACodec serializer,
                                     final byte[][] in, FSA root) throws IOException {
        final byte[] fsaData = serializer.write(root,
                new ByteArrayOutputStream()).toByteArray();

        FSA fsa = createCodec().read(new ByteArrayInputStream(fsaData));
        checkCorrect(in, fsa);
    }
    ...
    protected void checkCorrect(byte[][] input, FSA fsa) {
        ...
        for (ByteBuffer sequence : uniqueInput) {
            Assert.assertTrue(rl.remove(sequence), "Not present in the right language: " + sequence);
        }
    ...
    public void testAutomatonWithNodeNumbers() throws IOException {
        //Assume.assumeTrue(createCodec().getFlags().contains(FSAFlags.NUMBERS));
        ...
        final byte[] fsaData =
                createCodec()
                        .withNumbers()
                        .write(s, new ByteArrayOutputStream()).toByteArray();
     ...
     *
     */
    protected abstract FSACodec createCodec();
}
