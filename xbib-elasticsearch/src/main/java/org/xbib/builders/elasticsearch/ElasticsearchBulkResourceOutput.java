...
public class ElasticsearchBulkResourceOutput<C extends ResourceContext> extends ElasticsearchResourceOutput<C> {
    ...
    public ElasticsearchBulkResourceOutput() {
        this(100, 30);
    }
    
    public ElasticsearchBulkResourceOutput(int bulkSize, int maxActiveRequests) {
    ...
    protected AbstractWrite getOperator(String index, String type) {
        BulkWrite op = new BulkWrite(index, type);
        if (bulkSize > 0) {
            op.setBulkSize(bulkSize);
        }
        if (maxActiveRequests > 0) {
            op.setMaxActiveRequests(maxActiveRequests);
        }
        return op;
    }
}
