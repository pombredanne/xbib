...
public class ElasticsearchIndexerDAO<C extends ResourceContext>
        extends ElasticsearchDAO
        implements ElasticsearchIndexerInterface, ElementOutput<C> {

    private static final Logger logger = LoggerFactory.getLogger(ElasticsearchIndexerDAO.class.getName());
    ...
    @Override
    public ElasticsearchIndexerDAO settings(Settings settings) {
        super.settings(settings);
        return this;
    }

    @Override
    public ElasticsearchIndexerDAO newClient(URI uri, boolean force) {
    ...
    @Override
    public ElasticsearchIndexerDAO setIndex(String index) {
        super.setIndex(index);
        return this;
    }

    @Override
    public ElasticsearchIndexerDAO setType(String type) {
        super.setType(type);
        return this;
    }

    public ElasticsearchIndexerDAO setBulkSize(int bulkSize) {
        this.bulkSize = bulkSize;
        return this;
    }

    public ElasticsearchIndexerDAO setMaxActiveRequests(int maxActiveRequests) {
        this.maxActiveRequests = maxActiveRequests;
        return this;
    }

    public ElasticsearchIndexerDAO setMillisBeforeContinue(long millis) {
        this.millisBeforeContinue = millis;
        return this;
    }

    public ElasticsearchIndexerDAO write(Resource resource) throws IOException {
    ...
    public ElasticsearchIndexerDAO flush() throws IOException {
    ...
    public ElasticsearchIndexerDAO createIndex(String mapping) throws NoNodeAvailableException {
        client.admin().indices().create(new CreateIndexRequest(index[0])).actionGet();
        if (mapping != null) {
            client.admin().indices().putMapping(new PutMappingRequest().indices(index).type(type[0]).source(mapping)).actionGet();
        }
        return this;
    }

    public ElasticsearchIndexerDAO putMapping(String mapping) {
        client.admin().indices().putMapping(new PutMappingRequest().indices(index).type(type[0]).source(mapping)).actionGet();
        return this;
    }

    public ElasticsearchIndexerDAO deleteIndex() throws NoNodeAvailableException {
            ...
            currentBulk.get().execute(new ActionListener<BulkResponse>() {
                ...
                public void onResponse(BulkResponse bulkResponse) {
                    if (bulkResponse.hasFailures()) {
                        ...
                        final int totalActions = bulkCounter.addAndGet(numberOfActions);
